...
var CanvasPlasmid, CanvasWorkerPlasmidLL, Plasmid, Plasmid1D, Plasmid2D, PlasmidLL, _ref,
...
Plasmid = (function() {
  ...
  Plasmid.prototype.time = function() {
  ...
  Plasmid.prototype.dump = function(input, clone) {
    if (input == null) {
      input = false;
    }
    if (clone == null) {
    ...
    if (input instanceof Array && input[0] instanceof Array) {
      if (input.length === this.cells.length && input[0].length === this.cells[0].length) {
        this.cells = clone ? this.clone(input) : input;
        return this.render();
      } else {
...
CanvasPlasmid = (function(_super) {
  __extends(CanvasPlasmid, _super);

  function CanvasPlasmid(canvasWrapper) {
    this.canvasWrapper = canvasWrapper;
    this.color = ['#cccccc', '#4d4d4d'];
    CanvasPlasmid.__super__.constructor.call(this, this.canvasWrapper);
    this.toggleSaved = {
      x: -1,
      y: -1
    };
  ...
  CanvasPlasmid.prototype.refresh = function() {
    var heightMultiplier, widthMultiplier;
    this.step = 0;
    this.cells = this.matrix(this.row + 2, this.col + 2);
    this.canvasWrapper.empty();
    widthMultiplier = this.canvasWrapper.width() / this.col;
    heightMultiplier = this.canvasWrapper.height() / this.row;
    this.multiplier = this.int(Math.min(widthMultiplier, heightMultiplier));
    this.$canvas = $('<canvas />').attr({
      width: this.col * this.multiplier,
      height: this.row * this.multiplier
    }).appendTo(this.canvasWrapper);
    this.ctx = this.$canvas.get(0).getContext('2d');
    this.ctx.fillRect(0, 0, this.row, this.col);
    return this.ctx.scale(this.multiplier, this.multiplier);
  };

  CanvasPlasmid.prototype.render = function() {
    var i, j, _i, _ref, _results;
    this.ctx.fillStyle = this.color[0];
    this.ctx.fillRect(0, 0, this.col, this.row);
    this.ctx.fillStyle = this.color[1];
    _results = [];
    for (i = _i = 1, _ref = this.col; 1 <= _ref ? _i <= _ref : _i >= _ref; i = 1 <= _ref ? ++_i : --_i) {
      _results.push((function() {
        var _j, _ref1, _results1;
        _results1 = [];
        for (j = _j = 1, _ref1 = this.row; 1 <= _ref1 ? _j <= _ref1 : _j >= _ref1; j = 1 <= _ref1 ? ++_j : --_j) {
          if (this.cells[i][j] === 1) {
            _results1.push(this.ctx.fillRect(i - 1, j - 1, 1, 1));
          } else {
  ...
  CanvasPlasmid.prototype.drawCell = function(x, y) {
    this.ctx.fillStyle = this.color[this.cells[x][y]];
    return this.ctx.fillRect(x - 1, y - 1, 1, 1);
  };

  CanvasPlasmid.prototype.toggleCell = function(x, y) {
    this.cells[x][y] = 1 - this.cells[x][y];
    return this.drawCell(x, y);
  };

  CanvasPlasmid.prototype.toggleCellIfNew = function(x, y, reset) {
    if (reset == null) {
      reset = false;
    }
    x = 0 | (x / this.multiplier) + 1;
    y = 0 | (y / this.multiplier) + 1;
    if (this.toggleSaved.x !== x || this.toggleSaved.y !== y || reset) {
      this.toggleCell(x, y);
      this.toggleSaved.x = x;
      return this.toggleSaved.y = y;
    }
  };

  return CanvasPlasmid;
...
CanvasWorkerPlasmidLL = (function(_super) {
  __extends(CanvasWorkerPlasmidLL, _super);

  function CanvasWorkerPlasmidLL(canvas) {
    var birth, i, rule, survive, _i, _j, _k, _l, _len, _len1;
    this.canvas = canvas;
    this.worker = new Worker('js/plasmid_worker_ll.js');
    this.refreshFlag = false;
    CanvasWorkerPlasmidLL.__super__.constructor.call(this, this.canvas);
    rule = {
      survive: [],
      birth: []
    };
  ...
  CanvasWorkerPlasmidLL.prototype.refresh = function() {
    CanvasWorkerPlasmidLL.__super__.refresh.call(this);
    this.stop();
    return this.render();
  };

  CanvasWorkerPlasmidLL.prototype.stop = function() {
    this.worker.onmessage = function() {};
    return this.eventListening = false;
  };

  CanvasWorkerPlasmidLL.prototype.propagate = function(callback) {
    var messageHandler, _this;
    if (callback == null) {
      callback = false;
    }
    _this = this;
    CanvasWorkerPlasmidLL.__super__.propagate.call(this);
    ...
    if (!this.eventListening || (this.callback !== false && callback === false) || (this.callback === false && callback !== false)) {
